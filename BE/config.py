"""
Configuration for the server and map generation.
...
ALLOWED_WANTED_PAPER_ORIENTATIONS = [
    MapOrientation.AUTOMATIC.value, MapOrientation.LANDSCAPE.value, MapOrientation.PORTRAIT.value]
...
def FUNC_MM_TO_POINTS_CONVERSION(v): return max(
    v / MM_TO_INCH * MATPLOTLIB_POINTS_PER_INCH, MIN_WIDTH_POINTS)


def ALPHA_CLAMP_VALUE(v): return min(1, max(v, 0))
def ICON_EDGE_RATIO_MAPPING(v): return max(v, 0)
def EDGE_RATIO_MAPPING(v): return max(v + 1, 0)
def ZOOM_LEVEL_CLAMP_VALUE(v): return min(10, max(v, 1))
