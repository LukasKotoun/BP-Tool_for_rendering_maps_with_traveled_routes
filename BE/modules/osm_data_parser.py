...
class OsmDataParser():
    ...
    def __init__(self, wanted_nodes: WantedCategories, wanted_nodes_from_area: WantedCategories, wanted_ways: WantedCategories, wanted_areas: WantedCategories,
        ...
        self.area_columns = list(wanted_areas.keys()) + \
            areas_additional_columns
    ...
    def create_gdf(self, file_name: str, fromCrs: str, toCrs: str | None = None) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame, gpd.GeoDataFrame]:
        ...
        class ElementsFilter():
            ...
            def node(self, node: Node):
                return not (OsmDataParser._apply_filters(self.outer_self.wanted_nodes, node.tags))

            def way(self, way: Way):
                return not (OsmDataParser._apply_filters(self.outer_self.wanted_ways, way.tags))

            def area(self, area: Area):
                return not (OsmDataParser._apply_filters(self.outer_self.wanted_areas, area.tags))
        ...
        if (self.wanted_nodes):
            ...
            nodes_gdf = GdfUtils.create_gdf_from_file_processor(
                fp_node, fromCrs)
        else:
        ...
        if (self.wanted_nodes_from_area):
            fp_node_from_area: osmium.FileProcessor = osmium.FileProcessor(file_name)\
                ...
                .with_filter(ElementsFilter(self))\
                .with_filter(osmium.filter.GeoInterfaceFilter(tags=self.nodes_columns))
            nodes_from_area_gdf = GdfUtils.create_gdf_from_file_processor(
                fp_node_from_area, fromCrs)
        else:
            nodes_from_area_gdf = GdfUtils.create_empty_gdf(fromCrs)
        nodes_gdf = GdfUtils.combine_gdfs(
            [nodes_gdf,  GdfUtils.create_points_from_polygons_gdf(nodes_from_area_gdf)])
        ...
        if (self.wanted_areas):
            ...
            areas_gdf = GdfUtils.create_gdf_from_file_processor(
                fp_area, fromCrs)
        else:
