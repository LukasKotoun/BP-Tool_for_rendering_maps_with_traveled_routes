...
class GpxManager:
    ...
    def load_to_gdf_from_memory(gpx_files: list[UploadFile], groups: dict[str, list[str]], toCrs: str) -> gpd.GeoDataFrame:
        ...
        for file in gpx_files:
            ...
            gpx_gdf[GpxColumns.GROUP.value] = GpxManager.find_group_name(
                file.filename, groups)
            gpx_list.append(gpx_gdf.to_crs(toCrs))

        if (gpx_list):
            gpxs_gdf = GdfUtils.combine_gdfs(gpx_list)
            GdfUtils.remove_columns(gpxs_gdf, [
                                    'geometry', GpxColumns.FILE_NAME.value, GpxColumns.GROUP.value], neg=True)
