...
class GpxProcesser:
    ...
    def get_gpxs_gdf(self, toEpsg: int) -> gpd.GeoDataFrame:
        gpx_list: list[gpd.GeoDataFrame] = []
        for file in os.listdir(self.gpx_folder):
            if not file.endswith('.gpx'):
                continue
            gpx_gdf: gpd.GeoDataFrame = (gpd.read_file(os.path.join(self.gpx_folder, file), layer='tracks'))
            gpx_gdf['name'] = file
            gpx_list.append(gpx_gdf.to_crs(epsg=toEpsg))
        return gpd.GeoDataFrame(pd.concat(gpx_list, ignore_index=True), crs=f"EPSG:{toEpsg}")
