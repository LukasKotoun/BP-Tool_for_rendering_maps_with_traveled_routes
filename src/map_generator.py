...
def calc_preview(map_area_gdf, paper_dimensions_mm):
    ...
    outer_area_gdf = GdfUtils.get_whole_area_gdf(OUTER_AREA, EPSG_OSM, EPSG_CALC)
    ...
    if(not WANT_AREA_CLIPPING):
        ...
        map_area_gdf = GdfUtils.create_gdf_from_bounds(paper_fill_bounds, EPSG_CALC)
    else:
...
def main():
    ...
    nodes_gdf, ways_gdf, areas_gdf = osm_file_parser.create_gdf(EPSG_OSM, EPSG_DISPLAY)
    osm_file_parser.clear_gdf()

    map_area_gdf = map_area_gdf.to_crs(f"epsg:{EPSG_DISPLAY}")
    boundary_map_area_gdf = boundary_map_area_gdf.to_crs(f"epsg:{EPSG_DISPLAY}")
    ...
    if(WANT_AREA_CLIPPING or WANT_PREVIEW):
        plotter.clip(EPSG_DISPLAY, GdfUtils.create_polygon_from_gdf_bounds(nodes_gdf, ways_gdf, areas_gdf))

    if(boundary_map_area_gdf is not None):
        plotter.plot_area_boundary(area_gdf=boundary_map_area_gdf.to_crs(epsg=EPSG_DISPLAY), linewidth=AREA_BOUNDARY_LINEWIDTH)
