<script lang="ts">
  ...
  const searchAreaWhisper = async (query: string, index: number): Promise<void> => {
    if (!query || query.length < 1) {
      areaSuggestions[index] = [];
      return;
    }
    ...
    //todo do env
      axios.get(import.meta.env.VITE_API_NOMINATIM_URL,{
        ...
        if (response.status === 200) {
          ...
          areaSuggestions[index] = data.map(item => item.display_name);
        }
  ...
  const selectAreaSuggestion = (displayName: string, index: number): void => {
    $wantedAreas[index].area = displayName;
    areaSuggestions[index] = [];
    ...
    <div class="space-y-4 rounded-lg bg-gray-100 ">
      ...
      {#each $wantedAreas as area (area.id)}
          <div class="p-4 flex flex-wrap gap-4 items-start">
            ...
            <div class="flex flex-col">
          <p class="text-sm font-medium mb-1">Získat nápovědu</p>
              <button 
                class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded"
                class:bg-gray-500={gettingSuggestions}
                class:hover:bg-gray-600={gettingSuggestions}
                on:click={() => searchAreaWhisper(area.area, area.id)}
