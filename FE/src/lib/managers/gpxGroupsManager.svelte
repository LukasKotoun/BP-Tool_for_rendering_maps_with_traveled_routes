<script lang="ts">
  ...
  function createGroup() {
    ...
    $gpxStyles = {
      ...
      group: {
        ...$gpxStyles.group,
        [groupName]: JSON.parse(JSON.stringify(gpxDefaultStyles)),
      },
    };
  ...
  function removeGroup(groupName: string) {
    //remove from styles
    const { [groupName]: removedGroup, ...remainingGroupsStyles } =
      $gpxStyles.group;
  ...
  function removeFileFromGroup(groupName: string, fileName: string) {
    $gpxFileGroups[groupName] = $gpxFileGroups[groupName].filter(
      (file) => file !== fileName
    );
  }
</script>
