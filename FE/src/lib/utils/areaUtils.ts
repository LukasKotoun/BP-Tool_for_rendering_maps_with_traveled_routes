...
export function checkPaperDimensions(
  ...
  allow_one_dimension_only: boolean = false
): boolean {
...
export function parseWantedArea(input: string): number[][] | string {
  ...
  let coordinates = pairs.map((pair) => {
    ...
    if (values.length !== 2) {
      throw new Error(
        `Invalid coordinate pair: ${pair} - must have exactly two values separated by comma`
      );
    }
...
export async function searchAreaWhisper(query: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    ...
    axios
      .get(import.meta.env.VITE_API_NOMINATIM_URL, {
        params: {
          format: "json",
      ...
      .then((response) => {
        if (response.status === 200) {
          const polygonResults = response.data.filter((result: any) => {
            return result.osm_type && result.osm_type === "relation";
          });
