...
class GdfUtils:
    ...
    def lines_to_polygons():
        pass#remove unwanted spaces between lines in corners - by converting to polygon and expanding 
...
class GeoDataStyler:
    def __init__(self, categories_styles, general_default_styles):        
        self.categories_styles = categories_styles
        self.general_default_styles = general_default_styles
    
    def assign_styles_to_row(self, row, available_styles):
        assigned_styles = {}
    ...
    def filter_category_styles(self, wanted_styles, wanted_categories):
    ...
    def assign_styles_to_gdf(self, gdf, filters_for_gdf_categories, wanted_styles):
        gdf_available_styles = self.filter_category_styles(wanted_styles, filters_for_gdf_categories)
        styles_columns = gdf.apply(lambda row: self.assign_styles_to_row(row, gdf_available_styles), axis=1).tolist()
...
class MapPlotter:
    def __init__(self, GdfUtils, ways_gdf, areas_gdf, reqired_map_area, general_default_styles):
        self.GdfUtils = GdfUtils
    ...
    def plot_ways(self):
        self.ways_gdf.plot(ax=self.ax, color=self.ways_gdf['color'], alpha=1)
...
gdf_styler = GeoDataStyler(CATEGORIES_STYLES, GENERAL_DEFAULT_STYLES)
ways_gdf = gdf_styler.assign_styles_to_gdf(ways_gdf, way_filters, ['color', 'zindex', 'linewidth'])
areas_gdf = gdf_styler.assign_styles_to_gdf(areas_gdf, area_filters, ['color', 'zindex'])
...
map_plotter = MapPlotter(GdfUtils, ways_gdf, areas_gdf, reqired_map_area, GENERAL_DEFAULT_STYLES)
